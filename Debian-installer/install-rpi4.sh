...
if [[ -n "$APPIMAGE_FILE_OVERRIDE" ]]; then
  ...
  echo "Downloading KiOSC-BrowsR from explicit URL..."
...
if [[ -f "$LABWC_SYSTEM_AUTOSTART" ]]; then
  ...
  if grep -q '^/usr/bin/lwrespawn /usr/bin/wf-panel-pi &$' "$LABWC_SYSTEM_AUTOSTART"; then
    sudo sed -i 's|^/usr/bin/lwrespawn /usr/bin/wf-panel-pi &$|# disabled by KiOSC-BrowsR: /usr/bin/lwrespawn /usr/bin/wf-panel-pi \&|' "$LABWC_SYSTEM_AUTOSTART"
  fi
fi
